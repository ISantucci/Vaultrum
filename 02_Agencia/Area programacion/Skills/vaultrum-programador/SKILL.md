...
Registralas así: Dónde aterriza: `<Proyecto>/05_Programacion/`, según la regla **Dónde aterriza cada salida** de `02_Indice Agencia`. La ruta del proyecto sale del cuaderno; **nunca se escribe adentro de `Vaultrum/`**. Si no hay carpeta de proyecto, no la inventes: devolvé a Producción. Actualizá el cuaderno del proyecto. Antes de numerar, revisá los índices. `SOL/EJ` comparten número base con su `RQ`, y su relación es **1:N**: la forma exacta la fija `02_Indice Agencia`, que es su autoridad. Lo único que se repite acá es lo que más se olvida: si un `SOL` cubre varios `RQ`, **los enumera en su `Insumo`**, y hay un instrumento que lo verifica. Linkeá siempre hacia atrás, incluyendo el `LDS`/`UXS` si existieron.
